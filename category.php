<?php
	session_start();
    if(!isset($_GET['categoryid']) || !$_GET['categoryid'] || !is_numeric($_GET['categoryid'])){
        ...
        $_GET['categoryid'] = intval($_GET['categoryid']);
                    ...
                    # 判断该分类是否存在
                    $sql_statement = "SELECT category_id FROM categories WHERE category_id = '" . $_GET['categoryid'] ."'" ;
        ...
        <div class="row">
            <main class="col-md-8">
            	...
            	<?php
            		...
            		# 数据库查询文章列表
            		$query = "SELECT user_id, user_name, article_id, article_title, article_post_datetime, article_category_id, category_name, article_excerpt, article_click FROM users, articles, categories WHERE article_category_id='".$_GET["categoryid"]."' AND category_id=article_category_id AND article_user_id=user_id ORDER BY article_post_datetime DESC";
...
EOT;
            		...
            		$result -> close();
                    if(!isset($flag)){
                        echo '<div class="no-post"> 该分类中暂时还没有发布文章</div>';
                    }
            	?>
