<?php
    ...
    if(isset($_GET['userid']) && is_numeric($_GET['userid']) && isset($_SESSION["isLogin"]) && isset($_SESSION["usertype"]) && $_SESSION["usertype"] == 1){
    	...
    	# 查询用户
    	$sql_statement = "SELECT user_name FROM users WHERE user_type='0' AND user_lock='1' AND user_id='" . $_GET['userid'] ."'";
		...
		if($result -> num_rows == 1){
			...
			# 修改数据库
			 ...
			 echo '用户: <b>'.$username.'</b>成功被解封<br>三秒后自动跳转管理页面~~~';
		}else{
			header("refresh:3;url=../manageuser.php"); 
			echo '该用户未被封禁或该用户不存在<br>三秒后自动跳转管理页面~~~';
		}
    }else{
